...
export default function OrderFilter({ option }: any) {
  ...
  const handleLowToHigh = () => {
    if (ascending === "desc") {
      handleOrder({ ...router.query, ascending: "asc" });
      return;
    }
    handleOrder({ ...router.query, ascending: "desc" });
    return;
  };
    ...
    <div className="flex items-center text-xl w-full justify-between">
      ...
      <div className="flex items-center">
        ...
        >
          ...
          <option className="appearance-none" value="title">
            Name
          </option>
        </select>
        <button onClick={handleLowToHigh}>
          <svg
            ...
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={`${lowToHigh.ascending === "asc" && "rotate-180"}`}
          >
